...
def objective(trial):
    ...
    model = FLfastRegressor(number_of_intervals=number_of_intervals,
    threshold=threshold,
    metric = metric).fit(X=X_train,y=y_train,X_valid=None,y_valid=None)
    y_pred_train = model.predict(X=X_train)
    r2 = r2_score(y_train, y_pred_train)
    return r2 

study = optuna.create_study(direction = "maximize", pruner=optuna.pruners.MedianPruner(),sampler=TPESampler())
...
model = FLfastRegressor(number_of_intervals=trial.params['number_of_intervals'],threshold=trial.params['threshold'],metric = trial.params['metric']).fit(X=X_train,y=y_train,X_valid=None,y_valid=None)
