...
class FLAutoOptunaClassifier:
    ...
    def _optimizer_func(self,*args, **kwargs):
        ...
        def objective(trial):
            ...
            number_of_intervals = trial.suggest_int("number_of_intervals", int(number_of_intervals[0]),int(number_of_intervals[len(number_of_intervals)-1]))
            threshold= trial.suggest_float("threshold", float(threshold[0]),float(threshold[len(threshold)-1]))
            fuzzy_type= trial.suggest_categorical("fuzzy_type", fuzzy_type)
            if fuzzy_type=='triangular':
                fuzzy_cut= trial.suggest_float("fuzzy_cut", float(fuzzy_cut[0]),float(fuzzy_cut[len(fuzzy_cut)-1]))
            params = {
                "metric": metric,
...
class FLOptunaClassifier:
    ...
    def _optimizer_func(self,*args, **kwargs):
        ...
        def objective(trial):
            ...
            # Define selection parameter
            metric= trial.suggest_categorical("metric", metric)
            number_of_intervals = trial.suggest_int("number_of_intervals", number_of_intervals[0],number_of_intervals[len(number_of_intervals)-1])
            threshold= trial.suggest_float("threshold", threshold[0],threshold[len(threshold)-1])
            fuzzy_type= trial.suggest_categorical("fuzzy_type", fuzzy_type)
            if fuzzy_type=='triangular':
                fuzzy_cut= trial.suggest_float("fuzzy_cut", float(fuzzy_cut[0]),float(fuzzy_cut[len(fuzzy_cut)-1]))
