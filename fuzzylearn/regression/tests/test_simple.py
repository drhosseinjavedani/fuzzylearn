...
X_train, X_test, y_train, y_test =train_test_split(X, y["target"], test_size=test_size, random_state=random_state)
...
model = FLRegressor(number_of_intervals=10,threshold=0.5,metric = 'manhattan').fit(X=X_train,y=y_train,X_valid=None,y_valid=None)
...
print(r2_score(y_test.to_list(), y_pred))
